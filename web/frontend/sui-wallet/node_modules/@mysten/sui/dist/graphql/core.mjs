...
var GraphQLCoreClient = class extends CoreClient {
	...
	async listOwnedObjects(options) {
		const objects = await this.#graphqlQuery({
			...
			variables: {
				...
				filter: options.type ? { type: (await this.mvr.resolveType({
					type: options.type,
					signal: options.signal
				})).type } : void 0,
				includeContent: options.include?.content ?? false,
	...
	async listCoins(options) {
		...
		const coins = await this.#graphqlQuery({
			...
			variables: {
				...
				cursor: options.cursor,
				first: options.limit,
				type: `0x2::coin::Coin<${(await this.mvr.resolveType({
					type: coinType,
					signal: options.signal
				})).type}>`
			}
	...
	async getReferenceGasPrice(options) {
		return { referenceGasPrice: await this.#graphqlQuery({
	...
	async getProtocolConfig(options) {
		const result = await this.#graphqlQuery({
			...
			signal: options?.signal
		}, (result$1) => result$1.epoch?.protocolConfigs);
		const featureFlags = {};
	...
	async listTransactions(options) {
		...
		const transactions = await this.#graphqlQuery({
			...
			variables: {
				filter: filter && {
					sentAddress: filter.$kind === "sender" ? filter.sender : void 0,
					function: filter.$kind === "function" ? [
						filter.package,
						filter.module,
						filter.function
					].filter(Boolean).join("::") : void 0
				},
				first: descending ? void 0 : limit,
				after,
				last: descending ? limit : void 0,
				before,
				includeTransaction: options.include?.transaction ?? false,
				includeEffects: options.include?.effects ?? false,
				includeEvents: options.include?.events ?? false,
		...
		return {
			transactions: (descending ? [...transactions.nodes].reverse() : transactions.nodes).map((transaction) => parseTransaction(transaction, options.include)),
			hasNextPage: descending ? transactions.pageInfo.hasPreviousPage : transactions.pageInfo.hasNextPage,
			startCursor: (descending ? transactions.pageInfo.endCursor : transactions.pageInfo.startCursor) ?? null,
			endCursor: (descending ? transactions.pageInfo.startCursor : transactions.pageInfo.endCursor) ?? null
		};
	}
	async listEvents(options) {
		...
		const events = await this.#graphqlQuery({
			...
			variables: {
				filter: filter && {
					sender: filter.$kind === "sender" ? filter.sender : void 0,
					module: filter.$kind === "emitModule" ? `${filter.package}::${filter.module}` : void 0,
					type: filter.$kind === "eventTypeModule" ? `${filter.package}::${filter.module}` : filter.$kind === "eventType" ? filter.eventType : void 0
				},
				first: descending ? void 0 : limit,
				after,
				last: descending ? limit : void 0,
		...
		return {
			events: (descending ? [...events.nodes].reverse() : events.nodes).map((event) => {
				...
				if (!packageId || !module || !sender || !eventType || !transactionDigest) throw new Error("listEvents response is missing expected event fields");
				return {
	...
	async getMoveFunction(options) {
		const moveFunction = await this.#graphqlQuery({
			...
			signal: options.signal,
			variables: {
				package: (await this.mvr.resolvePackage({
